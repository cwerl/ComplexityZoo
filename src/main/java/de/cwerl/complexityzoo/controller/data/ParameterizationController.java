...
public class ParameterizationController {
    ...
    public String viewParam(Model model, @PathVariable("id") long id, @PathVariable("paramId") long paramId) {
        ...
        model.addAttribute("ctpCandidates", ctpRepository.findAllComplexityClassCandidatesOrdered(paramId,
                ComplexityDataType.Values.PARAMETERIZED));
        return "problems/params/view";
    }

    @PreAuthorize("isAuthenticated()")
    @PostMapping(value = "/new/save")
    public String newParamSave(@PathVariable long id, @RequestParam String description, @RequestParam String name,
            Model model) {
    ...
    public String editParamSave(@PathVariable long id, @PathVariable long paramId, @RequestParam String description) {
        parameterizationRepository.updateDescription(paramId,
